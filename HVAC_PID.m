...
xlim('tight')
ylabel('Temperature')
legend([p1,p4,p2,p3],'Model identification PID','Cost optimal PID', 'Reference interval','Reference value','Location','best')

subplot(4,1,3); hold on;
h22 = plot(u,'r');
ax = gca;
ax.XAxis.Exponent = 4;
legend([h21,h22],'Model identification PID','Cost optimal PID','Location','best')
ylabel({'Control','signal'})
xlim('tight')

subplot(4,1,4); hold on;
h32 = plot(u_sum,"color",'r');
text(length(u_sum),u_sum(end),['$\leftarrow$',num2str(ceil(u_sum(end)))],'FontSize',8)
ax = gca;
ax.XAxis.Exponent = 4;
ylabel('Cost')
legend([h31,h32],'Model identification PID','Cost optimal PID','Location','best')
xlabel('Time step')
xlim('tight')

    figure(1);
    name = '..\HVAC_Images\HVAC_PID_comparison.pdf';
    exportgraphics(gcf,name,'BackgroundColor','none');
    pause(0.0001)
