    ...
    for iSim = k_0:Nsim
        ...
        while ((term > termMax) && (iter<max_iter))
            ...
            for i = 1 : swarm_size
                ...
                val = (ymH-ymRefH).^2  + lambda*sum(ux.^2) + gamma*sum(eb.^2) + weightU + weightUz; % izraèun kriterijske funkcije
...
xlim('tight')
subplot(3,1,2); hold off;
plot((Q_D_p(1:length(U)-1) + M.act_P*Q_P_p(1:length(U)-1)'),"color",color(4,:)); hold on;
plot((Q_I_p(1:length(U)-1)),'r'); 
legend('Disturbance', 'Control','location','best')
ylabel({'Thermal','energy'})
xlim('tight')
