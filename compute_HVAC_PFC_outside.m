function  [Yp, Ym, U, W, V, Z, u_sum, Q_D_p, Q_I_p, wlb, wub] = compute_HVAC_PFC_outside(Deltau, lambda_1, lambda_2, ar, H, alpha, n_var_0, var_0)
...
Nsim = length(w);
dw = 2;
wlb = [w;repelem(w(end),H,1)] - dw;
wub = [w;repelem(w(end),H,1)] + dw;
w_shift = 200;
...
U = 0*ones(k_0-1,1); Ym = T_z_m; Yp = T_z_p;  W = T_z_p;  uZ=0;
V = sig*ones(k_0-1,1);
Z = 2*ones(k_0-1,1);
...
for iSim = k_0:Nsim
    ...
    swarm(:,3,1) = swarm(:,1,1);
    %swarm(:,3,2) = swarm(:,1,2);
    swarm(:, 4, 1) = 1e5;          % najboljša vrednost do sedaj
    %swarm(:, 4, 2) = NaN;          % najboljša vrednost do sedaj
    swarm(:, 2, :) = 0;            % zaèetna hitrost
    term = 1000; termMax = 1e-5;

    %Izdelaj ustavitveni pogoj
    max_iter = 100;
    iter = 0;
    eb_error = 0;
    error_weight = 1e5;
    while ((term > termMax) && (iter<max_iter))
        ...
        for i = 1 : swarm_size
            ...
            ymH = T_z_m_H(h0+H);
            delta_m = (T_z_m_H(h0:h0+H) - T_z_m_H(h0));
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            elb = wlb(iSim:(iSim+H))-(ypH- z_sig*sig);
            elb = max(elb, 0);
            eub = (ypH + z_sig*sig)-wub(iSim:(iSim+H));
            eub = max(eub, 0);
            eb = elb + eub;

            val = lambda_1*((ux).^2) + lambda_2*sum(eb); % izraèun kriterijske funkcije
        ...
        %for i = 1 : swarm_size
        swarm(:, 2, :) = rand*inertia*swarm(:, 2, :) + correction_factor*rand*(swarm(:, 3, :) - swarm(:, 1, :)) ...
            + correction_factor*rand*(swarm(gbest, 3, :) - swarm(:, 1, :));   %x komponenta hitrosti
        %end
...
%             figure(3);
%             clf
%             %plot((swarm(:,1,1)-mean(swarm(:,1,1)))/std(swarm(:,1,1)),(val-mean(val))/std(val), 'x')   %  izris premikanja delca%
%             plot(swarm(:,1,1), val, 'x')   %  izris premikanja delca%
%             pause(.00001)
%         end
